...
addGlobalStyle(() => {
	...
	return css`
		...
		button f-icon:first-child, button f-icon-loading:first-child{
			margin-left: ${-(lineHeight / 10 - 1)}px;
			margin-right: ${lineHeight / 5 - 2}px;
		}
	
		button f-icon:last-child, button f-icon-loading:last-child{
		...
		button[round] f-icon, button[round] f-icon-loading{
			margin-left: auto;
			margin-right: auto;
		}
		...
		.f-touched.f-valid, .f-touched:f-valid{
			box-shadow: inset 0 -2px 0 0 ${successColor};
		}
		
		.f-touched.f-invalid, .f-touched:f-invalid{
